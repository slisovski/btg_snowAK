...
fls.gz <- list.files("/bioing/user/slisovsk/4km/", pattern = ".gz", recursive = T,  full.names = T)
...
snowM <- do.call("cbind", pbmclapply(1:length(dates), function(x) {  
    ...
    unlist(raster::extract(S01, crdsP))
