...
fls.gz <- list.files("/Users/slisovsk/Desktop/4km", pattern = ".asc.gz", recursive = T,  full.names = T)
...
prj <- "+proj=stere +lat_0=90 +lat_ts=60 +lon_0=-80 +k=1 +x_0=0 +y_0=0 +a=6378137 +b=6356257 +units=m +no_defs"
  ...
  crdsP <- coordinates(S02)[unlist(extract(S02, as(btg %>% st_transform(CRS(prj)) %>% st_geometry(), "Spatial"))),]
...
for(i in 1:length(dates)) {
    ...
    snowM[,i] <- unlist(extract(S01, crdsP))
